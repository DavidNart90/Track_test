...
from src.trackrealties.analytics.search import SearchAnalytics, search_analytics
            ...
            if filters:
                for key, value in filters.items():
                    filter_clauses.append(
                        f"metadata->>'{key}' = ${len(filter_values) + 3}"
                    )
...
class OptimizedGraphSearch(FixedGraphSearch):
    ...
    async def search(
        ...
        try:
            ...
            if entities["locations"]:
                for location in entities["locations"]:
                    if "," in location:
                        ...
                        market_results = await self._search_market_data_safe(
                            city, state, limit
                        )
            ...
            if entities["metrics"] and entities["locations"]:
                metric_results = await self._search_metrics_safe(
                    entities["metrics"], entities["locations"], limit
                )
    ...
    async def _search_market_data_safe(
        self, city: str, state: str, limit: int
    ) -> list[dict]:
    ...
    async def _search_metrics_safe(
        self, metrics: list[str], locations: list[str], limit: int
    ) -> list[dict]:
        ...
        try:
            async with self.driver.session() as session:
                ...
                result = await session.run(
                    query, metrics=metrics, locations=locations, limit=limit
                )
                return await result.data()
        except Exception as e:
...
class OptimizedHybridSearch:
    ...
    async def search(
        ...
        try:
            ...
            logger.info(
                f"Hybrid search combined {len(vector_results)} vector + {len(graph_results)} graph results"
            )
    ...
    def _intelligent_fusion(
        ...
        sorted_results = sorted(
            result_map.values(), key=lambda x: x["score"], reverse=True
        )
